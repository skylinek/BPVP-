...
public class Modular {
        ...
        for (int i = 0; i < sizesModulus.length; i = i + 1) {
            ...
            for (int j = 0; j < sizes.length; j = j + 1) {
                ...
                if (mod == 0) {
                    ...
                    Test_MOD_EXP_ALL(counter, opp1, opp2, modulus, sizes[j] / 8, sizesModulus[i] / 8);
                } else if (mod == 2) {
     ...
     *
     */
    public void Test_MOD_EXP_ALL(int counter, byte[] opp1, byte[] opp2, byte[] modulus, int sizeOPP, int sizeModulus) throws Exception {
        /* if (opp1.length != modulus.length) {
            return;
        }
